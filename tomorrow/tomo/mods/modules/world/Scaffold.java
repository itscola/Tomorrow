...
enum TowerMode {
	Vanilla, HypixelSmooth, Packet
}

public class Scaffold extends Module {
	public Mode mode = new Mode<>("Mode", "Mode", ScMode.values(), ScMode.Hypixel);
	...
	private final Mode towerMode = new Mode("TowerMode", "TowerMode", TowerMode.values(), TowerMode.HypixelSmooth);
	...
	private void doMotionUp() {
		if (this.getBlockCount() > 0 && this.tower.getValue() && (!PlayerUtils.isMoving() || towerMove.getValue())) {
			...
			if (PlayerUtils.isMoving()) {
				...
				if (this.towerMode.getValue().toString().equals("HypixelSmooth")) {
					if (this.towerTick < 14) {
						if (PlayerUtils.isOnGround(0.76) && !PlayerUtils.isOnGround(0.75) && mc.thePlayer.motionY > 0.23
								&& mc.thePlayer.motionY < 0.25) {
							mc.thePlayer.motionY = Math.round(mc.thePlayer.posY) - mc.thePlayer.posY;
						}
						...
						if (PlayerUtils.isOnGround(0.9) && !PlayerUtils.isOnGround(0.89) && mc.thePlayer.motionY > 0.23
								&& mc.thePlayer.motionY < 0.25) {
							mc.thePlayer.motionY = Math.round(mc.thePlayer.posY) - mc.thePlayer.posY;
						}
					} else {
				...
				} else if (this.towerMode.getValue().toString().equals("Vanilla")) {
					mc.thePlayer.jumpMovementFactor = 0.0f;
					if (this.isAirBlock(this.block) && this.blockData != null) {
					...
					if (this.towerTick >= 2) {
						mc.thePlayer.sendQueue.addToSendQueue(new C03PacketPlayer.C04PacketPlayerPosition(
								mc.thePlayer.posX, mc.thePlayer.posY + 0.42D, mc.thePlayer.posZ, false));
						mc.thePlayer.sendQueue.addToSendQueue(new C03PacketPlayer.C04PacketPlayerPosition(
								mc.thePlayer.posX, mc.thePlayer.posY + 0.748D, mc.thePlayer.posZ, false));
