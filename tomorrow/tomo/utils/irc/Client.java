...
public class Client {
    ...
    public static void connect(String userName, String gameID) {
        ...
        try {
            ...
            connected = true;
            sendMessage(IRCUtils.toJson(new ClientConnectPacket(System.currentTimeMillis(), userName+" connected", userName, gameID)));
            ...
            if (packet.type.equals(IRCType.CHAT)) {
                ...
                if (Helper.mc.thePlayer != null) {
                    ...
                    if (ModuleManager.getModuleByClass(IRC.class).isEnabled()) {
                        Helper.sendMessageWithoutPrefix(ChatFormatting.BLUE + "[IRC]" + ChatFormatting.WHITE + " " + c.content);
                    }
                }
            } else if (packet.type.equals(IRCType.HEART)) {
                timerUtil.reset();
                ServerHeartNeededPacket c = (ServerHeartNeededPacket) IRCUtils.toPacket(ircMessage, ServerHeartNeededPacket.class);
                User u = (User) IRCUtils.toJson(c.content,User.class);
                user.authName = u.authName;
                user.hwid = u.hwid;
                user.connected = u.connected;
                user.password = u.password;
                System.out.println("ÐÄÔàÌø¶¯£¡" + user.authName + user.GameID + user.head);
                ...
                if (Helper.mc.thePlayer != null) {
                    assert ModuleManager.getModuleByClass(IRC.class) != null;
                    if (ModuleManager.getModuleByClass(IRC.class).isEnabled()) {
                        Helper.sendMessageWithoutPrefix(ChatFormatting.BLUE + "[IRC]" + ChatFormatting.RED + " " + c.content);
                    }
                }
            }
